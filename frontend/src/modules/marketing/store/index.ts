/**
 * @module marketing/store
 * @description Marketing 状态管理
 */
...
export const useMarketingStore = defineStore('marketing', {
  ...
  actions: {
    async fetchList(params?: any) {
      this.loading = true
      try {
        const { data } = await marketingApi.list(params)
        this.list = data.data || []
    ...
    async fetchById(id: string) {
      const { data } = await marketingApi.detail(id)
      this.current = data.data
    },

    async create(data: any) {
      const { data: result } = await marketingApi.create(data)
      this.list.unshift(result.data)
      return result
    },

    async update(id: string, data: any) {
      const { data: result } = await marketingApi.update(id, data)
