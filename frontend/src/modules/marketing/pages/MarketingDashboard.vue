<!-- 
  文件路径: frontend/src/modules/marketing/pages/MarketingDashboard.vue
  功能: 营销中心首页 - 市场营销总览
-->

<template>
  <div class="page-container">
    <el-card class="header-card">
      <div class="page-header">
        <div>
          <h2>营销中心</h2>
          <p class="subtitle">市场营销活动管理</p>
        </div>
        <div>
          <el-date-picker
            v-model="dateRange"
            type="daterange"
            range-separator="至"
            start-placeholder="开始日期"
            end-placeholder="结束日期"
            size="large"
            style="width: 280px"
          />
          <el-button type="primary" size="large" @click="handleRefresh" style="margin-left: 12px;">
            <el-icon><Refresh /></el-icon> 刷新
          </el-button>
        </div>
      </div>
    </el-card>

    <!-- KPI卡片 -->
    <el-row :gutter="20" class="kpi-row">
      <el-col :span="6" v-for="kpi in marketingKpis" :key="kpi.label">
        <el-card class="kpi-card" :class="kpi.trend">
          <div class="kpi-header">
            <span class="kpi-label">{{ kpi.label }}</span>
            <el-tag :type="kpi.trend === 'up' ? 'success' : 'danger'" size="small">
              {{ kpi.trend === 'up' ? '↑' : '↓' }} {{ Math.abs(kpi.change) }}%
            </el-tag>
          </div>
          <div class="kpi-value">{{ kpi.value }}</div>
          <div class="kpi-footer">较上期 {{ kpi.change > 0 ? '增长' : '下降' }} {{ Math.abs(kpi.change) }}%</div>
        </el-card>
      </el-col>
    </el-row>

    <!-- 营销入口 -->
    <el-row :gutter="20">
      <el-col :span="6" v-for="item in marketingEntries" :key="item.label">
        <el-card class="entry-card" @click="handleNavigate(item.path)">
          <div class="entry-icon" :style="{ background: item.color }">
            <el-icon :size="32"><component :is="item.icon" /></el-icon>
          </div>
          <div class="entry-label">{{ item.label }}</div>
          <div class="entry-desc">{{ item.desc }}</div>
          <div class="entry-action">
            <el-button type="primary" text>进入 →</el-button>
          </div>
        </el-card>
      </el-col>
    </el-row>

    <!-- 图表区域 -->
    <el-row :gutter="20" style="margin-top: 20px">
      <el-col :span="16">
        <el-card class="chart-card">
          <template #header><span>营销效果趋势</span></template>
          <div ref="trendChartRef" class="chart-container"></div>
        </el-card>
      </el-col>
      <el-col :span="8">
        <el-card class="chart-card">
          <template #header><span>渠道效果</span></template>
          <div ref="pieChartRef" class="chart-container"></div>
        </el-card>
      </el-col>
    </el-row>
  </div>
</template>

<script setup lang="ts">
import { ref, onMounted, nextTick } from 'vue'
import { Refresh, TrendCharts, Promotion, Share, DataLine, Monitor, User, Edit } from '@element-plus/icons-vue'
import { useRouter } from 'vue-router'
import { ElMessage } from 'element-plus'
import * as echarts from 'echarts'

const router = useRouter()
const dateRange = ref<[Date, Date]>([new Date(new Date().setDate(new Date().getDate() - 30)), new Date()])

const marketingKpis = ref([
  { label: '营销活动数', value: '86', change: 12.5, trend: 'up' },
  { label: '总曝光量', value: '1,285,600', change: 18.3, trend: 'up' },
  { label: '点击转化率', value: '8.5%', change: 2.1, trend: 'up' },
  { label: '营销ROI', value: '285%', change: 5.6, trend: 'up' },
])

const marketingEntries = ref([
  { label: '营销活动', desc: '管理营销活动', icon: 'Promotion', color: '#409EFF', path: '/marketing/campaigns' },
  { label: '邮件营销', desc: '邮件营销管理', icon: 'Message', color: '#67C23A', path: '/marketing/email' },
  { label: '社交媒体', desc: '社交媒体管理', icon: 'Share', color: '#E6A23C', path: '/marketing/social' },
  { label: '线索生成', desc: '销售线索管理', icon: 'User', color: '#9B59B6', path: '/marketing/leads' },
])

const trendChartRef = ref<HTMLElement>()
const pieChartRef = ref<HTMLElement>()

const handleRefresh = () => { ElMessage.success('刷新完成') }
const handleNavigate = (path: string) => { router.push(path) }

const initCharts = async () => {
  await nextTick()
  if (trendChartRef.value) {
    const chart = echarts.init(trendChartRef.value)
    chart.setOption({
      tooltip: { trigger: 'axis' },
      legend: { data: ['曝光量', '点击量', '转化率'] },
      grid: { left: '3%', right: '4%', bottom: '3%', containLabel: true },
      xAxis: { type: 'category', data: ['1月', '2月', '3月', '4月', '5月', '6月'] },
      yAxis: [
        { type: 'value', name: '曝光量', splitLine: { lineStyle: { color: '#f0f0f0' } } },
        { type: 'value', name: '转化率(%)', min: 0, max: 20, splitLine: { show: false } },
      ],
      series: [
        { name: '曝光量', type: 'bar', data: [120, 200, 150, 180, 220, 260], itemStyle: { color: '#409EFF' } },
        { name: '点击量', type: 'line', data: [12, 20, 15, 18, 22, 26], lineStyle: { color: '#67C23A', width: 3 }, smooth: true },
        { name: '转化率', type: 'line', yAxisIndex: 1, data: [6.5, 7.2, 7.8, 8.2, 8.8, 9.5], lineStyle: { color: '#E6A23C', width: 3, type: 'dashed' }, smooth: true },
      ],
    })
    window.addEventListener('resize', () => chart.resize())
  }
  if (pieChartRef.value) {
    const chart = echarts.init(pieChartRef.value)
    chart.setOption({
      tooltip: { trigger: 'item', formatter: '{b}: {c} ({d}%)' },
      series: [{
        type: 'pie',
        radius: ['40%', '70%'],
        data: [
          { value: 35, name: '邮件营销', itemStyle: { color: '#409EFF' } },
          { value: 25, name: '社交媒体', itemStyle: { color: '#67C23A' } },
          { value: 20, name: '内容营销', itemStyle: { color: '#E6A23C' } },
          { value: 12, name: '线下活动', itemStyle: { color: '#F56C6C' } },
          { value: 8, name: '其他', itemStyle: { color: '#909399' } },
        ],
        label: { formatter: '{b}\n{d}%' },
        emphasis: { scale: true },
      }],
    })
    window.addEventListener('resize', () => chart.resize())
  }
}

onMounted(() => { initCharts() })
</script>

<style scoped>
.page-container { padding: 20px; background: #f5f7fa; min-height: 100vh; }
.header-card { border-radius: 12px; }
.page-header { display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px; }
.page-header h2 { margin: 0; font-size: 20px; }
.subtitle { color: #909399; margin: 4px 0 0 0; }
.kpi-row { margin-bottom: 20px; }
.kpi-card { border-radius: 12px; transition: all 0.3s; }
.kpi-card.up { border-left: 4px solid #67C23A; }
.kpi-card.down { border-left: 4px solid #F56C6C; }
.kpi-header { display: flex; justify-content: space-between; align-items: center; }
.kpi-label { color: #909399; font-size: 14px; }
.kpi-value { font-size: 28px; font-weight: 700; color: #303133; margin: 8px 0; }
.kpi-footer { color: #909399; font-size: 12px; }
.entry-card { text-align: center; border-radius: 12px; cursor: pointer; transition: all 0.3s; padding: 16px 0; }
.entry-card:hover { transform: translateY(-4px); box-shadow: 0 8px 25px rgba(0,0,0,0.12); }
.entry-icon { width: 56px; height: 56px; border-radius: 12px; margin: 0 auto 12px; display: flex; align-items: center; justify-content: center; color: white; }
.entry-label { font-size: 16px; font-weight: 600; }
.entry-desc { color: #909399; font-size: 13px; margin: 4px 0; }
.entry-action { margin-top: 8px; }
.chart-card { border-radius: 12px; }
.chart-container { height: 280px; width: 100%; }
</style>