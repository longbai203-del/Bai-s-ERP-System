...
export const useinventoryStore = defineStore('inventory', () => {
  ...
  async function fetchList(params: any = {}) {
    ...
    try {
      // API 调用
      const result = await apiService.get('/inventory', {
  ...
  async function fetchById(id: string) {
    ...
    error.value = null;
    try {
      const result = await apiService.get(`/inventory/${id}`);
  ...
  async function create(data: any) {
    ...
    error.value = null;
    try {
      const result = await apiService.post('/inventory', data);
      await fetchList();
      return result;
    } catch (err: any) {
      error.value = err.message;
  ...
  async function update(id: string, data: any) {
    ...
    error.value = null;
    try {
      const result = await apiService.put(`/inventory/${id}`, data);
      await fetchList();
      return result;
    } catch (err: any) {
      error.value = err.message;
