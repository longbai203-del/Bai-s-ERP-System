<!-- 
  文件路径: frontend/src/modules/analytics/pages/Index.vue
  功能: 分析列表 - 管理所有分析报告
-->
  ...
  <div class="analytics-index-page">
    ...
    <div class="page-header">
      <div class="header-left">
        <h1>📊 分析报告管理</h1>
        <p class="header-subtitle">管理所有分析报告，支持多种分析类型</p>
      </div>
    ...
    <el-card class="table-card" shadow="hover">
      ...
      >
        ...
        <el-table-column prop="type" label="分析类型" width="120" align="center">
          <template #default="{ row }">
    ...
    >
      <div class="delete-confirm">
        ...
        <p>确定要删除分析报告 <strong>{{ deleteTarget?.name }}</strong> 吗？</p>
...
import {
  ...
  TrendCharts, User, Box, DataLine, Connection, Histogram
...
const statsData = ref([
  { label: '总报告', value: '156', icon: 'TrendCharts', color: '#409EFF' },
  { label: '销售分析', value: '48', icon: 'TrendCharts', color: '#67C23A' },
  { label: '客户分析', value: '32', icon: 'User', color: '#E6A23C' },
  { label: '产品分析', value: '28', icon: 'Box', color: '#F56C6C' },
])
...
const tableData = ref([
  ...
  },
  {
    ...
    name: '月度销售趋势分析',
    type: 'trend',
    createdBy: 'Abdullah Al-Shammari',
...
const loadData = async () => {
  ...
  try {
    ...
    total.value = 156
    // 实际项目中调用 API
    // const res = await api.get('/analytics', { params: { page: currentPage.value, limit: pageSize.value, ...searchForm } })
...
const handleEdit = (row: any) => {
  router.push(`/analytics/${row.id}/edit`)
}
...
[data-theme="dark"] .analytics-index-page {
  background: #1A1A2E;
}
