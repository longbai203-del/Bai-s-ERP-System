...
export const useAnalyticsStore = defineStore('analytics', {
  ...
  actions: {
    async fetchList(params?: any) {
      this.loading = true
      try {
        const { data } = await analyticsApi.list(params)
        this.list = data.data || []
    ...
    async fetchById(id: string) {
      const { data } = await analyticsApi.detail(id)
      this.current = data.data
    },

    async create(data: any) {
      const { data: result } = await analyticsApi.create(data)
      this.list.unshift(result.data)
      return result
    },

    async update(id: string, data: any) {
      const { data: result } = await analyticsApi.update(id, data)
