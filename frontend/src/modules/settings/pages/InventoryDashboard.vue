<!--
  文件路径: frontend/src/modules/settings/pages/InventoryDashboard.vue
  功能: 系统设置仪表板
    ...
    <div class="page-header">
      <div class="header-left">
        <el-breadcrumb separator="/">
          ...
          <el-breadcrumb-item v-if="pageType !== 'List' && pageType !== 'Dashboard'">系统设置仪表板</el-breadcrumb-item>
        </el-breadcrumb>
        <h1 class="page-title">系统设置仪表板</h1>
      </div>
...
const loadData = async () => {
  loading.value = true;
  try {
    const response = await settingsApi.getList(filters);
...
const loadDetail = async (id: string) => {
  loading.value = true;
  try {
    const data = await settingsApi.getDetail(id);
...
const handleView = (id: string) => router.push(/settings/);
const handleCreate = () => router.push(/settings/create);
const handleEdit = (id?: string) => {
  const targetId = id || currentItem.value?.id || route.params.id;
  if (targetId) router.push(/settings//edit);
};
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (pageType.value === 'Edit' && currentItem.value?.id) {
      await settingsApi.update(currentItem.value.id, data);
...
const handleDelete = async (id?: string) => {
  ...
  try {
    ...
    await settingsApi.delete(targetId);
    ElMessage.success('删除成功');
    if (pageType.value === 'Detail') router.push(/settings);
    else loadData();
  } catch (error) {
