...
export const actions: ActionTree<purchaseState, any> = {
    async fetchData({ commit }, params?: any) {
        try {
            ...
            commit(purchaseMutations.SET_DATA, mockData)
            commit(purchaseMutations.SET_TOTAL, mockData.length)
            
        } catch (error: any) {
            commit(purchaseMutations.SET_ERROR, error.message || '获取数据失败')
        } finally {
    ...
    async fetchDetail({ commit }, id: number) {
        try {
            ...
            // commit(purchaseMutations.SET_CURRENT_ITEM, response.data)
            
            const mockItem = { id, name: '示例详情', createdAt: new Date().toISOString() }
            commit(purchaseMutations.SET_CURRENT_ITEM, mockItem)
            
        } catch (error: any) {
            commit(purchaseMutations.SET_ERROR, error.message || '获取详情失败')
        } finally {
            commit(purchaseMutations.SET_LOADING, false)
        }
    },
    ...
    async createData({ commit }, data: any) {
        try {
            ...
            commit(purchaseMutations.SET_ERROR, error.message || '创建失败')
            throw error
        } finally {
            commit(purchaseMutations.SET_LOADING, false)
        }
    ...
    async updateData({ commit }, { id, data }: { id: number; data: any }) {
        try {
            ...
            commit(purchaseMutations.SET_ERROR, error.message || '更新失败')
            throw error
        } finally {
            commit(purchaseMutations.SET_LOADING, false)
        }
    ...
    async deleteData({ commit }, id: number) {
        try {
            ...
            commit(purchaseMutations.SET_ERROR, error.message || '删除失败')
            throw error
        } finally {
            commit(purchaseMutations.SET_LOADING, false)
        }
    ...
    setPage({ commit }, { page, pageSize }: { page: number; pageSize: number }) {
        commit(purchaseMutations.SET_PAGE, { page, pageSize })
    },
