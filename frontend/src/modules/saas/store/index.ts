...
export const useSaasStore = defineStore('saas', {
  ...
  actions: {
    async fetchList(params?: any) {
      this.loading = true
      try {
        const { data } = await saasApi.list(params)
        this.list = data.data || []
    ...
    async fetchById(id: string) {
      const { data } = await saasApi.detail(id)
      this.current = data.data
    },

    async create(data: any) {
      const { data: result } = await saasApi.create(data)
      this.list.unshift(result.data)
      return result
    },

    async update(id: string, data: any) {
      const { data: result } = await saasApi.update(id, data)
