...
export const actions: ActionTree<ordersState, any> = {
    async fetchData({ commit }, params?: any) {
        try {
            ...
            commit(ordersMutations.SET_DATA, mockData)
            commit(ordersMutations.SET_TOTAL, mockData.length)
            
        } catch (error: any) {
            commit(ordersMutations.SET_ERROR, error.message || '获取数据失败')
        } finally {
    ...
    async fetchDetail({ commit }, id: number) {
        try {
            ...
            // commit(ordersMutations.SET_CURRENT_ITEM, response.data)
            
            const mockItem = { id, name: '示例详情', createdAt: new Date().toISOString() }
            commit(ordersMutations.SET_CURRENT_ITEM, mockItem)
            
        } catch (error: any) {
            commit(ordersMutations.SET_ERROR, error.message || '获取详情失败')
        } finally {
            commit(ordersMutations.SET_LOADING, false)
        }
    },
    ...
    async createData({ commit }, data: any) {
        try {
            ...
            commit(ordersMutations.SET_ERROR, error.message || '创建失败')
            throw error
        } finally {
            commit(ordersMutations.SET_LOADING, false)
        }
    ...
    async updateData({ commit }, { id, data }: { id: number; data: any }) {
        try {
            ...
            // TODO: 替换为实际的 API 调用
            // const response = await updateorders(id, data)
            // return response
            
            return { success: true, data: { id, ...data } }
            
        } catch (error: any) {
            commit(ordersMutations.SET_ERROR, error.message || '更新失败')
            throw error
        } finally {
            commit(ordersMutations.SET_LOADING, false)
        }
    ...
    async deleteData({ commit }, id: number) {
        try {
            ...
            commit(ordersMutations.SET_ERROR, error.message || '删除失败')
            throw error
        } finally {
            commit(ordersMutations.SET_LOADING, false)
        }
    ...
    setPage({ commit }, { page, pageSize }: { page: number; pageSize: number }) {
        commit(ordersMutations.SET_PAGE, { page, pageSize })
    },
