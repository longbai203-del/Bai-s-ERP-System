  ...
  <div class="pos-returns-page">
    ...
    <div class="page-header">
      <div class="header-left">
        <h1>
          <el-icon :size="28" color="#EF4444"><RefreshRight /></el-icon>
          退货管理
        </h1>
        <span class="subtitle">退货申请处理与退款跟踪</span>
      </div>
    ...
    <el-row :gutter="20" class="stat-cards">
      ...
      <el-col :xs="24" :sm="12" :lg="4">
        <el-card class="stat-card" shadow="hover" @click="filterByStatus('refunded')">
          <div class="stat-content">
...
const generateMockData = () => {
  const customers = ['张三', '李四', '王五', '赵六', '孙七', '周八', '吴九']
  const reasons = ['产品质量问题', '发错商品', '运输损坏', '客户不满意', '其他原因']
  const statuses = ['pending', 'approved', 'refunded', 'rejected', 'pending', 'refunded']
...
const calculateStats = (data: any[]) => {
  ...
  stats.refunded = data.filter(item => item.status === 'refunded').length
  stats.rejected = data.filter(item => item.status === 'rejected').length
  stats.totalAmount = data.reduce((sum, item) => sum + item.amount, 0)
}
...
const filterByStatus = (status: string) => {
  filterForm.status = status === 'all' ? '' : status
  pagination.current = 1
  filterData()
}
...
const handleViewDetail = (id: number) => {
  router.push(`/pos/returns/detail/${id}`)
}

const handleApprove = (row: any) => {
  ElMessageBox.confirm(`确定要批准退货 ${row.returnNo} 吗？`, '批准确认', {
    ...
    if (item) {
      ...
      calculateStats(allData.value)
      filterData()
      ElMessage.success(`退货 ${row.returnNo} 已批准`)
    }
  }).catch(() => {})
}
...
const handleRefund = (row: any) => {
  ElMessageBox.confirm(`确定要执行退款 ${row.returnNo} 吗？`, '退款确认', {
    ...
    if (item) {
      ...
      calculateStats(allData.value)
      filterData()
      ElMessage.success(`退款 ${row.returnNo} 已完成`)
    }
  }).catch(() => {})
}
...
const confirmDialogSubmit = async () => {
  ...
  try {
    ...
    const newItem = {
      ...
      returnNo: `RET-${String(allData.value.length + 1).padStart(6, '0')}`,
