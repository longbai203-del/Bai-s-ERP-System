...
export const useposStore = defineStore('pos', () => {
  ...
  async function fetchList(params: any = {}) {
    ...
    try {
      // API 调用
      const result = await apiService.get('/pos', {
  ...
  async function fetchById(id: string) {
    ...
    error.value = null;
    try {
      const result = await apiService.get(`/pos/${id}`);
  ...
  async function create(data: any) {
    ...
    error.value = null;
    try {
      const result = await apiService.post('/pos', data);
      await fetchList();
      return result;
    } catch (err: any) {
      error.value = err.message;
  ...
  async function update(id: string, data: any) {
    ...
    error.value = null;
    try {
      const result = await apiService.put(`/pos/${id}`, data);
      await fetchList();
      return result;
    } catch (err: any) {
      error.value = err.message;
