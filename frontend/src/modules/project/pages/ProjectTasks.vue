...
const tableData = ref([
  ...
  { id: 3, taskNo: 'TSK-005', name: '接口对接', project: '阿尔拉吉银行核心系统', assignee: 'Faisal Al-Dossary', priority: 'medium', status: 'todo', deadline: '2024-12-30' },
])
...
const handleCreate = () => { ElMessage.info('新建任务') }
const handleView = (row: any) => { ElMessage.info(`查看任务: ${row.name}`) }
const handleEdit = (row: any) => { ElMessage.info(`编辑任务: ${row.name}`) }
const handleComplete = (row: any) => {
  ElMessageBox.confirm(`确认完成任务 ${row.name}？`, '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'info' })
    .then(() => { row.status = 'done'; ElMessage.success('任务已完成') }).catch(() => {})
}
