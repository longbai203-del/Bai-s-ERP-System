...
export const useprojectStore = defineStore('project', () => {
  ...
  async function fetchList(params: any = {}) {
    ...
    try {
      // API 调用
      const result = await apiService.get('/project', {
  ...
  async function fetchById(id: string) {
    ...
    error.value = null;
    try {
      const result = await apiService.get(`/project/${id}`);
  ...
  async function create(data: any) {
    ...
    error.value = null;
    try {
      const result = await apiService.post('/project', data);
      await fetchList();
      return result;
    } catch (err: any) {
      error.value = err.message;
  ...
  async function update(id: string, data: any) {
    ...
    error.value = null;
    try {
      const result = await apiService.put(`/project/${id}`, data);
      await fetchList();
      return result;
    } catch (err: any) {
      error.value = err.message;
