<!-- 
  文件路径: frontend/src/modules/finance/pages/BudgetManagement.vue
  功能: 预算管理 - 管理年度/月度预算
-->
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="6">
            <el-form-item>
              ...
              <el-button type="primary" @click="handleCreate" style="float: right"><el-icon><Plus /></el-icon> 编制预算</el-button>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    ...
    <el-card>
      <el-table :data="tableData" v-loading="loading" style="width: 100%" stripe>
        ...
        <el-table-column label="执行率" align="center" width="150">
          <template #default="{ row }">
            <el-progress :percentage="row.executionRate" :color="row.executionRate < 70 ? '#67C23A' : row.executionRate < 90 ? '#E6A23C' : '#F56C6C'" />
          </template>
        </el-table-column>
        <el-table-column prop="status" label="状态" align="center" width="100">
          <template #default="{ row }">
            <el-tag :type="row.executionRate < 70 ? 'success' : row.executionRate < 90 ? 'warning' : 'danger'">
              {{ row.executionRate < 70 ? '正常' : row.executionRate < 90 ? '预警' : '超支' }}
            </el-tag>
          </template>
        </el-table-column>
...
const budgetStats = ref([
  ...
  { label: '已执行', value: 'SAR 5,856,000', type: 'warning' },
  { label: '剩余预算', value: 'SAR 2,644,000', type: 'success' },
  { label: '预算执行率', value: '68.9%', type: 'primary' },
])

const tableData = ref([
  { department: '销售部', annualBudget: 3000000, usedAmount: 1856000, remaining: 1144000, executionRate: 61.9 },
  { department: '采购部', annualBudget: 2500000, usedAmount: 1987000, remaining: 513000, executionRate: 79.5 },
  { department: '市场部', annualBudget: 2000000, usedAmount: 1765000, remaining: 235000, executionRate: 88.3 },
  { department: '财务部', annualBudget: 1000000, usedAmount: 450000, remaining: 550000, executionRate: 45.0 },
])
...
const handleCreate = () => { ElMessage.info('编制预算') }
