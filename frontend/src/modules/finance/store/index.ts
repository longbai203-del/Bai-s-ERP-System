/**
 * @module finance/store
 * @description Finance 状态管理
 */
...
export const useFinanceStore = defineStore('finance', {
  ...
  actions: {
    ...
    async fetchById(id: string) {
      const { data } = await financeApi.detail(id)
      this.current = data.data
    },

    async create(data: any) {
      const { data: result } = await financeApi.create(data)
      this.list.unshift(result.data)
      return result
    },

    async update(id: string, data: any) {
      const { data: result } = await financeApi.update(id, data)
