<template>
  <div class="edit-module-page" v-if="item">
    <div class="page-header">
      <div class="header-left">
        <router-link :to="`/new-module/${item.id}`" class="btn-back">
          ...
          <div class="form-group">
            ...
            <div v-if="form.attachments.length > 0" class="attachment-list">
              ...
              >
                ...
                <span v-if="file.url" class="file-status">已上传</span>
  ...
  setup() {
    ...
    const fetchDetail = async () => {
      ...
      try {
        ...
        Object.keys(form).forEach(key => {
          if (key === 'attachments') {
            form.attachments = response.data.attachments || [];
          } else if (key === 'tags') {
    ...
    const submitForm = async () => {
      ...
      try {
          ...
          if (key === 'attachments') {
            form.attachments.forEach((file, index) => {
              if (file.isNew && file.file) {
                ...
                formData.append(`existing_attachments[${index}]`, JSON.stringify(file));
              }
            });
          } else if (key === 'tags') {
        ...
        await api.post(`/new-module/${form.id}`, formData);
        showToast('更新成功！', 'success');
        router.push(`/new-module/${form.id}`);
      } catch (error) {
    ...
    const resetForm = () => {
      if (confirm('确定要重置所有修改吗？')) {
        Object.keys(form).forEach(key => {
          form[key] = originalData.value[key] || (Array.isArray(form[key]) ? [] : '');
        });
        tagInput.value = '';
        showToast('已重置', 'info');
      }
